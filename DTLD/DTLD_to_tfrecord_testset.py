...
def create_tf_example(example,appearances,width_limit):
  ...
  for box in example['objects']:
    ...
    if class_id == '3': # ignore label "Red-yellow"  
      continue 
    ...
    classes_plaintext.append(label_text)
  ...
  if widths: # if classes are not empty
    if [i for i in widths if i > 10]: # only keep the pictuers with width>10 objects to keep consistency of test set
      for cl in classes_plaintext:
        appearances[cl]+=1
